...
pub enum KnownBip32Path {
    ...
    /// Sign in with BitNames encryption.
    /// This path should be from the root.
    SiwbEncryptionFromRoot {
        epk_index: u32,
        service_bitname: BitName,
    },
    /// Sign in with BitNames encryption.
    /// This path should be from the encryption pubkey.
    SiwbEncryptionFromEpk {
        service_bitname: BitName,
    },
}

impl KnownBip32Path {
    pub const ENCRYPTION_PREFIX: [ChildNumber; 1] =
        [ChildNumber::Hardened { index: 1 }];
    pub const MESSAGE_SIGNING_PREFIX: [ChildNumber; 1] =
        [ChildNumber::Hardened { index: 2 }];
    pub const TX_SIGNING_PREFIX: [ChildNumber; 1] =
        [ChildNumber::Hardened { index: 0 }];

    const fn encryption(index: u32) -> [ChildNumber; 2] {
        let [prefix] = Self::ENCRYPTION_PREFIX;
        [prefix, ChildNumber::Hardened { index }]
    }

    const fn message_signing(index: u32) -> [ChildNumber; 2] {
        let [prefix] = Self::MESSAGE_SIGNING_PREFIX;
        [prefix, ChildNumber::Hardened { index }]
    }

    const fn tx_signing(index: u32) -> [ChildNumber; 2] {
        let [prefix] = Self::TX_SIGNING_PREFIX;
        [prefix, ChildNumber::Hardened { index }]
    }

    fn siwb_derivation_path(hardened: bool) -> [ChildNumber; 9] {
        let path_bytes: [u8; 32] =
            blake3::hash(b"Sign in with BitNames").into();
        derivation_path_from_bytes32(&path_bytes, hardened)
    }

    fn siwb_authentication(
        bitname: &BitName,
        service_bitname: &BitName,
        nonce: u64,
    ) -> Vec<ChildNumber> {
        let mut res = Vec::with_capacity(9 + 9 + 3 + 9);
        res.extend(Self::siwb_derivation_path(true));
        res.extend(derivation_path_from_bytes32(&bitname.0, true));
        res.extend(derivation_path_from_bytes8(nonce.to_le_bytes(), true));
        res.extend(derivation_path_from_bytes32(&service_bitname.0, false));
        res
    }

    fn siwb_encryption_from_epk(
        service_bitname: &BitName,
    ) -> impl Iterator<Item = ChildNumber> {
        Self::siwb_derivation_path(false)
            .into_iter()
            .chain(derivation_path_from_bytes32(&service_bitname.0, false))
    }

    fn siwb_encryption_from_root(
        epk_index: u32,
        service_bitname: &BitName,
    ) -> Vec<ChildNumber> {
        let mut res = Vec::with_capacity(2 + 9 + 9);
        res.extend(Self::encryption(epk_index));
        res.extend(Self::siwb_encryption_from_epk(service_bitname));
        res
    }
    ...
    pub fn derivation_path(&self) -> Vec<ChildNumber> {
        match self {
            ...
            Self::SiwbAuthentication {
                bitname,
                service_bitname,
                nonce,
            } => Self::siwb_authentication(bitname, service_bitname, *nonce),
            Self::SiwbEncryptionFromEpk { service_bitname } => {
                Self::siwb_encryption_from_epk(service_bitname).collect()
            }
            Self::SiwbEncryptionFromRoot {
                epk_index,
                service_bitname,
            } => Self::siwb_encryption_from_root(*epk_index, service_bitname),
        }
    }
}
...
mod tests {
    ...
    fn test_child_indexes_from_bytes8() -> anyhow::Result<()> {
        ...
        let expected_child_indexes = [0x83020100, 0x8e0c0a09, 0x00000002];
